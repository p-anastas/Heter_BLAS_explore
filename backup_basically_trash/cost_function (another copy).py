...
def linearize_cpu_to_gpu():
    ...
    for bound in bounds:
        bounded_times = []
        bounded_bytes = []
        for qbytes, qtime in zip(bytes, time):
            if (qbytes < bound and qbytes >= prev_bound):
                bounded_bytes.append(qbytes)
                bounded_times.append(qtime)
        x = np.array(bounded_bytes).reshape(-1, 1)
...
def plot_bw(plot_up_bound):
    ...
    import matplotlib.pyplot as plt
    plt.plot(bytes[0], GigaVal_per_s_l(bytes[0],time[0]), 'o', bytes[0], GigaVal_per_s_l(bytes[0], list(map(lambda x: t_memcpy(x,-1,0), bytes[0]))), '-', bytes[1], GigaVal_per_s_l(bytes[1],time[1]), 'o', bytes[1], GigaVal_per_s_l(bytes[1],list(map(lambda x: t_memcpy(x,0,-1), bytes[1]))), '-')
    # plt.xscale('log')
