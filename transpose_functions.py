...
float_bounds = [1e5,1e6,1e7,8e9]

resDir, _, trans_file = initialize()
...
def interpolate1d_transpose():
    ...
    f = interp1d(flops, time, kind='linear')
    return f

def linearize2d_transpose():
    (elems, time) = read_trans_file(trans_file)
    return LinearRegression_2d(elems,time,float_bounds)

def linearize_transpose():
    (elems, time) = read_trans_file(trans_file)
    flops = list(map(lambda x: x[0]*x[1], elems))
    return LinearRegression_1d(flops,time,float_bounds)
