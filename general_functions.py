...
def initialize():
    ...
    transfile = resDir + 'transpose_log_' + machine + '.md_sorted'
    cpugemmfile = resDir + 'CPU_only_log_' + machine + '.md_sorted'
    gpugemmfile = resDir + 'GPU_only_log_' + machine + '.md_sorted'
    daxpyfile = ''
    return (resDir,bwfile, transfile, cpugemmfile, gpugemmfile, daxpyfile)
...
def Bound_LinearRegression_1d(X_list,Y_list,lower_bound, upper_bound):
    ...
    if len(x) != 0:
        ...
        sys.exit('Bound_LinearRegression_1d: No benchmarks found for bound [%d,%d]' % (lower_bound,upper_bound))
    return (model, error)
...
def Bound_LinearRegression_2d(X_list,Y_list,lower_bound, upper_bound):
    ...
    if len(x) != 0:
        ...
        sys.exit('Bound_LinearRegression_2d: No benchmarks found for bound [%d,%d]' % (lower_bound,upper_bound))
    return (model, error)

def Bound_LinearRegression_3d(X_list,Y_list,lower_bound, upper_bound):
    bounded_Y = []
    bounded_X = []
    for qX, qY in zip(X_list, Y_list):
        if (qX[0]*qX[1]*qX[2] < upper_bound and qX[0]*qX[1]*qX[2] >= lower_bound):
            bounded_X.append(qX)
            bounded_Y.append(qY)
    x = np.array(bounded_X).reshape(-1, 3)
    y = np.array(bounded_Y)
    if len(x) != 0:
        model = LinearRegression(n_jobs=-1).fit(x,y)
        #r_sq = model.score(x, y)
        ...
        sys.exit('Bound_LinearRegression_3d: No benchmarks found for bound [%d,%d]' % (lower_bound,upper_bound))
    return (model, error)
...
def LinearRegression_3d(X_list,Y_list,bounds):
    linear_subparts = []
    prev_bound = 0
    err = 0
    for upper_bound in bounds:
        model,pred_err = Bound_LinearRegression_3d(X_list,Y_list,prev_bound, upper_bound)
        linear_subparts.append(model.predict) #.intercept_, model.coef_, lesser_bound, upper_bound)
        prev_bound = upper_bound
        err += pred_err
    err = err / len(bounds)
...
def GigaVal_per_s_l(l_val, l_time):    
    ...
    for val, time in zip(l_val, l_time):
        if time!= 0:
            ...
            sys.exit('Error: GigaVal_per_s_l called with elem val=%lf time=%lf' % (val,time))
        else:
            answer.append(0)        
    return answer
